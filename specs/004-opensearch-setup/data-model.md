...
**Purpose**: Document index mappings and document shapes for OpenSearch
...
| `created_at` | `date` | Yes | No | Yes | Record creation timestamp | ISO 8601 format |
| `updated_at` | `date` | Yes | No | Yes | Last update timestamp | Used for optimistic lock |
...
// Debezium CDC event (envelope)
{
  "payload": {
    ...
    "after": { "view_count": 1250500, "updated_at": "2024-06-20T08:12:00Z" }
  }
}
...
**Consumer Implementation** (feature 005):
```go
// Pseudo-code for consumer
documentID := cdcEvent.Payload.After.VideoID  // e.g., "video_12345"
indexRequest := opensearch.IndexRequest{
    Index:      "videos_index",
    DocumentID: documentID,  // Enables idempotent upsert
    Body:       marshalToJSON(cdcEvent.Payload.After),
}
```

**Optimistic Lock**:
- Compare `updated_at` from the CDC event against the stored document.
- Skip updates with older `updated_at` to avoid stale writes.
